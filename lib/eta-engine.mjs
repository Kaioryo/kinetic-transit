...
/**
 * ETA dalam DETIK — presisi penuh, tanpa pembulatan & tanpa lantai.
 *
 * Ini yang dipakai untuk PENGUKURAN (eta-logger → eta_logs). Jangan pakai
 * etaMinutesFor untuk mencatat prediksi: pembulatan ke menit + lantai 1 menit
 * akan MERUSAK evaluasi. Jarak antar-halte di sini pendek (waktu tempuh nyata
 * sering hanya 20-40 detik), jadi memaksa setiap prediksi jadi ≥60 detik membuat
 * error terukur didominasi artefak pembulatan, bukan oleh kualitas modelnya.
 *
 * `speedKmh` HARUS sudah bersih (hasil getMovementProfile). Nol yang TERUKUR
 * berarti bus tidak bergerak — kasus itu ditangani pemanggil lewat `isStopped`
 * (tidak menghasilkan ETA sama sekali), bukan ditambal di sini.
 */
export function etaSecondsFor(distanceKm, speedKmh) {
  return (distanceKm / Math.max(speedKmh, MIN_SPEED_KMH)) * 3600
}

/**
 * ETA dalam menit (bulat, minimal 1) — untuk TAMPILAN saja.
 *
 * Lantai 1 menit itu wajar bagi penumpang ("0 menit" tidak bermakna), tapi
 * merusak pengukuran. Untuk mencatat prediksi, pakai etaSecondsFor.
 */
export function etaMinutesFor(distanceKm, speedKmh) {
  return Math.max(1, Math.round(etaSecondsFor(distanceKm, speedKmh) / 60))
}
