...
import { haversineKm, stopDistances, etaMinutesFor, getMovementProfile } from './eta-engine.mjs'
 ...
 * @param opts    { tripId, lat, lng, stops, waypoints, state }
 *   stops: [{stop_id, name, lat, lng}] terurut; waypoints: array|null
 */
export async function processLocation(prisma, { tripId, lat, lng, stops, waypoints, state }) {
  ...
  if (!st) {
    ...
    state.set(tripId, st)
    await openPrediction(prisma, { tripId, lat, lng, stops, waypoints, st })
    return
  }
 ...
 */
async function openPrediction(prisma, { tripId, lat, lng, stops, waypoints, st }) {
  const target = stops[st.nextIdx]
  ...
  const etaMin = etaMinutesFor(distanceKm, movement.speedKmh)
